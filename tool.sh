...
uninstall_passwall() {
    ...
    printf "❓ 是否连同共享内核(Xray, Sing-Box, Hysteria, Geoview, ChinaDNS-NG)一起卸载清空？[y/N]: "
    read del_cores
    switch_cores=$(echo "$del_cores" | tr '[:upper:]' '[:lower:]')
    
    if [ "$switch_cores" = "y" ] || [ "$switch_cores" = "yes" ]; then
        apk del chinadns-ng xray-core sing-box hysteria geoview iptables-nft 2>/dev/null
    fi
    refresh_system
...
uninstall_argon() {
    ...
    echo "🔄 第一步：正在强制将 LuCI 网页外观主开关拨回原厂 Bootstrap..."
    uci set luci.main.mediaurlbase='/luci-static/bootstrap'
    uci commit luci
    
    echo "💥 第二步：正在物理拔除 Argon 主题包及中文后台管理面板..."
    apk del luci-theme-argon luci-app-argon-config luci-i18n-argon-config-zh-cn 2>/dev/null
    
    refresh_system
    echo "================================================="
    echo "✅ 🎉 还原完毕！系统网页外观已完美恢复官方原生毛坯房风格！"
    echo "💡 提示：现在去刷新你的网页浏览器即可见效。"
    echo "================================================="
}
...
while true; do
    ...
    case $choice in
        ...
        5) echo "👋 已退出。" ; exit 0 ;;
        *) echo "❌ 输入错误，请输入数字 1 到 5。" ; echo "" ; sleep 1 ;;
    esac
done
