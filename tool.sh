...
install_homeproxy() {
    ...
    if [ -z "$LATEST_VER" ]; then
        echo "❌ 警告：当前系统官方软件源内未发现 luci-app-homeproxy（官方源默认不收录代理插件）。"
        echo "-------------------------------------------------"
        echo "💡 我们可以尝试为您自动配置完美适配 APK v3 并绑定最终文件名的 HomeProxy 扩展源。"
        printf "❓ 是否允许脚本尝试为您配置第三方 HomeProxy 软件源？[y/N]: "
        read add_repo
        
        if [ "$add_repo" = "y" ] || [ "$add_repo" = "Y" ]; then
            ARCH=$(cat /etc/apk/arch 2>/dev/null || echo "aarch64_cortex-a53")
            HOMEPROXY_REPO_FILE="/etc/apk/repositories.d/homeproxy.list"
            
            mkdir -p /etc/apk/repositories.d
            rm -f "$HOMEPROXY_REPO_FILE" 2>/dev/null
            touch "$HOMEPROXY_REPO_FILE"
            
            # 写入含有 homeproxy 的 ImmortalWrt 兼容 Snapshots 专线，尾部死死锁死 packages.adb
            echo "https://downloads.immortalwrt.org/snapshots/packages/$ARCH/luci/packages.adb" > "$HOMEPROXY_REPO_FILE"
        ...
        if [ -z "$LATEST_VER" ]; then
            echo "❌ 错误：依旧未发现 luci-app-homeproxy，请手动检查网络或更换有效的自定义镜像源。"
            return 1
        fi
    fi
    ...
    if [ "$switch_confirm" = "y" ] || [ "$switch_confirm" = "yes" ]; then
        echo "🚀 正在通过 APK 引擎安全部署 HomeProxy、依赖内核及汉化包..."
        apk add --allow-untrusted \
                luci-app-homeproxy \
        ...
        if [ $? -eq 0 ]; then
            refresh_system
            echo "================================================="
            echo "✅ 🎉 恭喜老哥！HomeProxy 全套组件、Sing-Box 内核与汉化已完美闭环！"
            echo "💡 提示：现在去网页端（菜单：服务 -> HomeProxy）即可享受丝滑体验！"
            echo "================================================="
        else
            echo "❌ 安装失败，请查看上方 apk 报错。"
        fi
