...
sync_cert() {
    ...
    # 精准满足需求一：纯IP节点下的证书防御，不向 acme.sh 递交申请，直接在本地搓出自签证书
    if [ "$DOMAIN" = "$IP" ]; then
        ...
        echo "✅ 纯IP自签证书就位！(客户端连接时请务必开启允许不安全证书/允许跳过证书校验)"
        return 0
    fi
...
EOF_HY2_YAML
        ...
        # 精准满足需求二：剔除了“强走IPv4高速版”那一行输出。
        # 如果是纯IP，直接生成纯IP链接；如果是域名，生成常规域名版与IPv6测试版。
        if [ "$DOMAIN" = "$IP" ]; then
