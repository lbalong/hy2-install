...
info "系统架构: $ARCH, 匹配架构标签: $ARCH_TAG"
...
if [ ! -f /usr/local/bin/wgcf ]; then
  error "下载 wgcf 失败：文件未找到。"
fi
FILE_SIZE=$(wc -c < /usr/local/bin/wgcf)
info "wgcf 下载成功，文件大小: ${FILE_SIZE} 字节"
if [ "$FILE_SIZE" -lt 1000000 ]; then
  warn "警告：下载的 wgcf 文件过小（低于 1MB），可能下载到了错误或被拦截的页面。文件内容前 100 字符为："
  head -c 100 /usr/local/bin/wgcf || true
  echo ""
  error "wgcf 二进制不完整，请检查网络环境或 GitHub 连通性。"
fi
