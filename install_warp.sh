...
# Cloudflare WARP 固定公开的 WireGuard 对端参数（稳定，无需 API 获取）
CF_PUBLIC_KEY="bmXOC+F1FxEMF9dyiK2H5/1SUtzH0JuVo51h2wPfgyo="
CF_ENDPOINT="162.159.192.1:2408"
CF_DNS="1.1.1.1"
# 默认分配 IP（wgcf generate 失败时用此兜底，实际会被 WARP 服务器覆盖）
CF_ADDR4="172.16.0.2/32"
...
register_warp() {
  ...
  if "$WGCF_BIN" register --accept-tos 2>/dev/null; then
    ...
    warn "wgcf 在线注册失败，使用内置备用账户"
    # 来源：fscarmen warp 脚本内置公共账户，私钥已公开
    cat > "${WARP_CONF_DIR}/wgcf-account.toml" <<'TOML'
access_token = 'preset'
...
TOML
  ...
  [ -f "${WARP_CONF_DIR}/wgcf-account.toml" ] || die "wgcf-account.toml 不存在"

  # 尝试 wgcf generate 获取分配 IP（可能因 Cloudflare API 限制失败，失败则用内置参数）
  info "尝试获取 WARP 分配 IP（可选步骤，失败不影响使用）..."
  if "$WGCF_BIN" generate --force 2>/dev/null && [ -f "${WARP_CONF_DIR}/wgcf-profile.conf" ]; then
    info "wgcf-profile.conf 生成成功"
    WGCF_PROFILE="${WARP_CONF_DIR}/wgcf-profile.conf"
  else
    warn "wgcf generate 失败（Cloudflare API 对此 IP 限制），改用内置 WARP 参数直接连接"
    warn "功能不受影响，SOCKS5 代理将正常工作"
    WGCF_PROFILE=""
  fi
}
...
generate_wireproxy_conf() {
  ...
  local privkey addr4 addr6 pubkey endpoint dns reserved

  if [ -n "$WGCF_PROFILE" ] && [ -f "$WGCF_PROFILE" ]; then
    # 优先从 wgcf-profile.conf 读取（包含 Cloudflare 分配的真实 IP）
    info "从 wgcf-profile.conf 读取配置"
    privkey=$(  grep 'PrivateKey' "$WGCF_PROFILE" | head -1 | awk '{print $NF}')
    addr4=$(    grep 'Address'    "$WGCF_PROFILE" | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/[0-9]+' | head -1)
    addr6=$(    grep 'Address'    "$WGCF_PROFILE" | grep -oE '([0-9a-fA-F]{0,4}:){2,7}[0-9a-fA-F]{0,4}/[0-9]+' | head -1)
    dns=$(      grep 'DNS'        "$WGCF_PROFILE" | awk '{print $NF}' | head -1)
    pubkey=$(   grep 'PublicKey'  "$WGCF_PROFILE" | head -1 | awk '{print $NF}')
    endpoint=$( grep 'Endpoint'   "$WGCF_PROFILE" | head -1 | awk '{print $NF}')
    reserved=$( grep 'Reserved'   "$WGCF_PROFILE" | head -1 | awk -F'= ' '{print $2}')
  else
    # 从 wgcf-account.toml 提取私钥，其余用 Cloudflare 公开固定参数
    info "从 wgcf-account.toml 提取私钥，使用 Cloudflare 内置参数"
    local toml="${WARP_CONF_DIR}/wgcf-account.toml"
    [ -f "$toml" ] || die "找不到 wgcf-account.toml"
    privkey=$(grep 'private_key' "$toml" | head -1 | cut -d"'" -f2)
    [ -z "$privkey" ] && privkey=$(grep 'private_key' "$toml" | head -1 | cut -d'"' -f2)
    [ -z "$privkey" ] && die "无法从 wgcf-account.toml 提取私钥"
    addr4="$CF_ADDR4"
    dns="$CF_DNS"
    pubkey="$CF_PUBLIC_KEY"
    endpoint="$CF_ENDPOINT"
    reserved="[0, 0, 0]"
  fi

  # 最终兜底默认值
  privkey="${privkey:-hTk06uwwXhZx3RVqtug3MQ0RSodzdM/U5z/M5NIbh4c=}"
  pubkey="${pubkey:-${CF_PUBLIC_KEY}}"
  endpoint="${endpoint:-${CF_ENDPOINT}}"
  dns="${dns:-${CF_DNS}}"
  addr4="${addr4:-${CF_ADDR4}}"
...
CONF
  ...
  # 若有 IPv6 地址也加入
  if [ -n "${addr6:-}" ]; then
    sed -i "/^Address = /a Address = ${addr6}" "$WIREPROXY_CONF" 2>/dev/null || true
  fi

  info "wireproxy 配置写入完成: $WIREPROXY_CONF"
  echo ""
  info "配置预览:"
  grep -E '(PrivateKey|Address|PublicKey|Endpoint|BindAddress)' "$WIREPROXY_CONF" | sed 's/^/    /'
}
...
main() {
  ...
  WGCF_PROFILE=""   # 全局变量，register_warp 中赋值
  register_warp
  generate_wireproxy_conf
